...
@login_required
def dash_lab():
    user = db.first_or_404(sa.select(User).where(User.id == 1))
...
"""
Problema ao tentar iniciarlizar a tabela de comandos
O erro é referente a coluna 'comandos' ser desconhecida
"""
@app.route('/dash_lab/mudar_temp/<int:acao>')
@login_required
def mudar_temp(acao):
    if acao == 1: # aumentar temp
        comm = Comandos()
        comm.setup(current_user.id, "+Temp")
        db.session.add(comm)
        db.session.commit()
...
def mqtt():
    ...
    client.publish("/esp32/enviarComando", "Comando1")
