...
def index():
    """
    print("Tipo do usuario: ")
    print(type(current_user.tipo))
    print("Status do usuario: ")
    print(type(current_user.status))
    """

    if current_user.status == 1 and current_user.tipo == 1:
        print("aprovado e admin")
        sensores = db.session.scalar(sa.select(Sensores).where(Sensores.id == 1))

        print(sensores)

        return render_template('index_admin.html', title='Dash admin', sensores=sensores) # adicionar dados de sensores aqui
    
    if current_user.status == 1 and current_user.tipo == 0:
        print("aprovado e default")
